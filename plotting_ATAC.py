...
def plot_experiment_dfs(
    ...
    if mean_all_reps:
        ...
        mean_dfs_list = []
        var_df_list = []
        print(f'var type is: {variance_type}')
...
def plot_panel(
    ...
    if len(dfs_list) == 1:
        ...
        plt.title(f"{conditions}")
        # legend!!!
        for col_i in range(df.shape[1]):
            plot_vector(vec=df.iloc[:,col_i], ax=axes, color=colors(col_i), variance_vec=var_df.iloc[:,col_i])
...
def plot_vector(vec, ax, color, style="solid", variance_vec=0):
    ...
    """
    ax.plot(vec, c=color, linestyle=style)
    if variance_vec!=0:
        ax.fill_between(vec+variance_vec, vec-variance_vec, fc=color, alpha=0.2)
